...
{
    public class MusteriSiparisiController : BaseController
    {
        public MusteriSiparisiController(
            IHttpClientFactory httpClientFactory,
            IMemoryCache cache,
            IConfiguration config,
            ILogger<MusteriSiparisiController> logger)
        ...
        public async Task<IActionResult> List(int page = 1, int pageSize = 20)
        {
            try
            {
                ...
                var response = await http.GetAsync($"{ApiBaseUrl}/ItemSlips?limit={pageSize}&offset={offset}&docType=7");
            ...
            {
                _logger.LogError(ex, "MusteriSiparisi LIST hatasi");
        ...
        public async Task<IActionResult> GetFull(string fisNo)
        {
            ...
            {
                _logger.LogError(ex, "MusteriSiparisi GetFull hatasi");
        ...
        public async Task<IActionResult> Create([FromBody] JsonElement rawJson)
        {
            try
            {
                ...
                if (fatUst["Tarih"] == null || string.IsNullOrWhiteSpace((string?)fatUst["Tarih"])) fatUst["Tarih"] = now;
                if (fatUst["Tes_Tarih"] == null || string.IsNullOrWhiteSpace((string?)fatUst["Tes_Tarih"])) fatUst["Tes_Tarih"] = now;
                if (fatUst["Sube_Kodu"] == null || string.IsNullOrWhiteSpace(fatUst["Sube_Kodu"]?.ToString())) fatUst["Sube_Kodu"] = 0;
            ...
            {
                _logger.LogError(ex, "MusteriSiparisi Create hatasi");
                return StatusCode(500, new { error = ex.Message });
            }
        }
        ...
        public async Task<IActionResult> Save([FromBody] JsonElement rawJson)
        {
            try
            {
                ...
                var delResp = await http.DeleteAsync($"{ApiBaseUrl}/ItemSlips/7;{fisNo}");
                ...
                fullJson["FaturaTip"] = 7;
                fullJson["FatUst"]    ??= new JObject();
                var fatUst = (JObject)fullJson["FatUst"]!;
                fatUst["Tip"] = 7; fatUst["TIPI"] = 7; fatUst["AMBHARTUR"] = 0;
                fatUst["KDV_DAHILMI"] = true; fatUst["OZEL_KOD"] = "1";
            ...
            {
                _logger.LogError(ex, "MusteriSiparisi Save hatasi");
                return StatusCode(500, new { error = ex.Message });
            }
        }
        ...
        public async Task<IActionResult> Delete(string fisNo)
        {
            ...
            {
                var http = await CreateApiClientAsync();
                var response = await http.DeleteAsync($"{ApiBaseUrl}/ItemSlips/7;{fisNo}");
                return Content(await response.Content.ReadAsStringAsync(), "application/json");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "MusteriSiparisi Delete hatasi");
                return StatusCode(500, new { error = ex.Message });
            }
        }
    }
